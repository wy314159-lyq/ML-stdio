...
class TrainingModule(QWidget):
    ...
    def create_model_instance(self, model_class, random_state):
        ...
        # Use the smart model creation function
    ...
    def set_data(self, X: pd.DataFrame, y: pd.Series, previous_config=None):
        ...
        try:
            ...
            # CRITICAL FIX: Avoid double encoding by detecting if target is already encoded
            # If y is numeric and has small integer values, assume it's already encoded
            try:
                if pd.api.types.is_numeric_dtype(y):
                    ...
                    if is_likely_encoded_classification:
                        ...
                        print(f"✓ Detected pre-encoded classification target with {unique_values} classes")
                        print(f"Class labels: {self.class_names}")
                    elif unique_values < 20 and (unique_values / len(y)) < 0.05:
                        # Small number of unique values, likely classification but not sequential
                        self.task_type = "classification"
                        self.y = y.copy()
                        self.class_names = sorted(y.unique())
                        self.label_encoder = None
                        print(f"✓ Detected numeric classification target: {self.class_names}")
                    else:
                        ...
                        self.label_encoder = None
                        print(f"✓ Detected regression target (range: {y.min():.3f} - {y.max():.3f})")
                else:
            ...
            try:
                ...
                # Set default model from previous module if available
                if self.previous_model_selection and self.model_combo.findText(self.previous_model_selection) != -1:
                    self.model_combo.setCurrentText(self.previous_model_selection)
    ...
    def train_model(self):
        ...
        try:
            ...
            model_instance = self.create_model_instance(model_class, self.random_state_spin.value())
            ...
            print(f"DEBUG: After split and encoding:")
