...
class FeatureModule(QWidget):
    ...
    def update_available_models(self):
        ...
        try:
            # Get models from utils function
            available_models = get_available_models(task_type)
            model_names = list(available_models.keys())
            
            print(f"DEBUG: Found {len(model_names)} models: {model_names[:5]}...")  # Show first 5
            ...
            if self.model_combo.count() > 0:
                ...
                print(f"DEBUG: Selected default model: {self.model_combo.currentText()}")
            else:
                print("DEBUG: No models added to combo box!")
                
        except Exception as e:
            print(f"DEBUG: Error updating available models: {e}")
            import traceback
            traceback.print_exc()
            
            # Fallback to basic models
            basic_models = ["Random Forest", "Logistic Regression"] if task_type == "classification" else ["Random Forest", "Linear Regression"]
            print(f"DEBUG: Using fallback models: {basic_models}")
            for model_name in basic_models:
                self.model_combo.addItem(model_name)
                
        print(f"DEBUG: Final model combo count: {self.model_combo.count()}")
        print("DEBUG: update_available_models completed")
    ...
    def set_data(self, X: pd.DataFrame, y: pd.Series):
        """Set data from previous module with comprehensive validation"""
        ...
        # Detect task type from y values
        ...
        # CRITICAL FIX: Update available models based on detected task type
